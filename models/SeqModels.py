    ...
    ** Key methods **
    ...
    def attentive_inference_network_inputs(self):
        ...
        if self.inference_network in ['RETAIN', 'Seq2SeqAttention']: 
            ...
            Lengths_         = tf.repeat(self.length, self.maximum_seq_length)
    ...
    def get_corr(self, labels, pred):
 
        pred = np.argmax(pred, -1)
        labels_ = np.argmax(labels, -1)
        pos = np.where(np.mean(labels, axis=-1) == 0)[0]

        corr = (labels_ == pred)
        corr[pos] = False
        corr = np.sum(corr) 
        l = (np.mean(labels, axis=-1) !=0).sum()
    ...
    def stochastic_variational_inference(self, X, T=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            for i in range(0, len(X_), self.batch_size):
                ...
                batch_train   = X_[i:i + self.batch_size,:,:]
                batch_states  = state_update[i:i + self.batch_size,:,:]
                ...
                for k in range(batch_train.shape[0]):
                    corr, l = self.get_corr(batch_states[k,:,:], batch_preds[k,:,:])
                    corrs += corr
                    lens += l
            ...
            if self.verbosity:
                ...
                print('Epoch %d \t----- \tBatch %d \t----- \tLog-Likelihood %10.6e \t----- Acc %2.4f' % (epoch, _, np.mean(log_likelihood), acc))
