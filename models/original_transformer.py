...
class Transformer:
	def __init__(self, i_tokens, o_tokens, len_limit, d_model=256, d_data = 16,\
		...
		self.i_word_emb = TimeDistributed(Dense(self.d_model, use_bias=False,name = 'dense_emb'))
		...
		self.target_layer = TimeDistributed(Dense(d_data, use_bias=False))

	def compile(self, optimizer='adam', active_layers=999):
		# src_seq_input = Input(shape=(None,), dtype='int32')
		# tgt_seq_input = Input(shape=(None,), dtype='int32')

		src_seq_input = Input(shape=(None,self.d_data+1), dtype='float32')
		tgt_seq_input = Input(shape=(None,self.d_data+1), dtype='float32')

		src_seq = Lambda(lambda x:x[:,:,1:], name = 'enc_seq')(src_seq_input)
		src_time = Lambda(lambda x:x[:,:,0], name = 'enc_time')(src_seq_input)
		src_mask = Lambda(lambda x:tf.cast(tf.reduce_any(K.not_equal(x, 0), axis=-1), 'float32'), name = 'enc_mask')(src_seq)
		
		tgt_seq  =  Lambda(lambda x:x[:,:-1,1:])(tgt_seq_input)
		tgt_time = Lambda(lambda x:x[:,:-1])(src_time)
		tgt_mask = Lambda(lambda x:x[:,:-1])(src_mask)
		tgt_true = Lambda(lambda x:x[:,1:,1:])(tgt_seq_input)

		src_emb = self.i_word_emb(src_seq)
		tgt_emb = self.o_word_emb(tgt_seq)

		if self.pos_emb: 
			src_emb = add_layer([src_emb, self.pos_emb(src_time, pos_input=True)])
			tgt_emb = add_layer([tgt_emb, self.pos_emb(tgt_time, pos_input=True)])
		src_emb = self.emb_dropout(src_emb)

		enc_output = self.encoder(src_emb, src_mask, active_layers=active_layers)
		# dec_output = self.decoder(tgt_emb, tgt_mask, src_mask, enc_output, active_layers=active_layers)	
		...
		loss = get_loss(final_output, src_seq)
		# self.ppl = K.exp(loss)
