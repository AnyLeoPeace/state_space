...
def main():
    ...
    for proportion in [0.15]:
        ...
        eval_data = generate_data_mode_1(proportion=proportion, max_length=data_config['max_length'], min_seq=data_config['min_seq'], max_seq=data_config['max_seq'], personalized=test_personalized)
        ...
        for i in range(3):
            ...
            trans_scores = train_evaluate_my_model(train_data, eval_data, personalized=True, time_limit=data_config['max_length'], len_limit = data_config['max_seq'])
            # att_scores = train_evaluate_att_model(train_data, eval_data, len_limit = data_config['max_seq'])

            np.save('./results/personalized_+ ' + str(test_personalized)+ ' + proportion_' + str(proportion) + '_iteration_'+str(i),{'trans':trans_scores, 'proportion':(train_data[-1] / total, eval_data[-1] / total)})
